...
Mat PoissonBlender::getGradientXp(Mat &img){
    int height = img.rows;
    int width = img.cols;
...
Mat PoissonBlender::getGradientYp(Mat &img){
    int height = img.rows;
    int width = img.cols;
...
// calculate horizontal gradient, img(i, j - 1) - img(i, j)
Mat PoissonBlender::getGradientXn(Mat &img){
    int height = img.rows;
    int width = img.cols;
    Mat cat = repeat(img, 1, 2);
...
// calculate vertical gradient, img(i - 1, j) - img(i, j)
Mat PoissonBlender::getGradientYn(Mat &img){
    int height = img.rows;
    int width = img.cols;
    Mat cat = repeat(img, 2, 1);
...
int PoissonBlender::getLabel(int i, int j, int height, int width){
    return i * width + j;
}
...
// get Matrix A.
Mat PoissonBlender::getA(int height, int width){
...
// 1 -4  1
// 0  1  0
Mat PoissonBlender::getLaplacian(){
...
// Calculate b
// using convolution.
Mat PoissonBlender::getB1(Mat &img1, Mat &img2, int posX, int posY, Rect ROI){
    Mat Lap;
...
Mat PoissonBlender::getB2(Mat &img1, Mat &img2, int posX, int posY, Rect ROI){
...
Mat PoissonBlender::getResult(Mat &A, Mat &B, Rect &ROI){
    Mat result;
    solve(A, B, result);
...
// img1: 3-channel image, we wanna move something in it to img2.
// img2: 3-channel image, dst image.
// ROI: the position and size of the img1 block we want to move to img2.
// posX, posY: where we want to move the block to in img2
Mat PoissonBlender::poisson_blending(Mat &img1, Mat &img2, Rect ROI, int posX, int posY){
    ...
    merge(result, 3, merged);
    return merged;
}

//-------------------------------------------------------------------------------

int PoissonBlender::unit_test()
{
    const string path("D:/PRJ/Benchmark/OBJECTIVE_TEST/src/");

    const int error_x = 4;
    const int shift_x = 236 - error_x;

    long start, end;
    start = clock();

    Mat img1, img2;
    Mat in1 = imread(path + string("eye1.jpg"));
    Mat in2 = imread(path + string("hand1.jpg"));
 ...
 //   Rect roi = Rect(shift_x, 0, img1.cols - shift_x, img1.rows);
  //  Mat result = poisson_blending(img1, img2, roi, 0, 0);

    Rect roi = Rect(20, 20, 80, 80);
    Mat result = poisson_blending(img1, img2, roi, 15, 20);
    result.convertTo(result, CV_8UC1);

//    Rect roi2 = Rect(15, 50, 180, 180);
//    Mat roimat = in2(roi2);
