...
double ssim(Mat & img_src, Mat & img_compressed, int block_size, bool show_progress)
{
    ...
    for (int k = 0; k < nbBlockPerHeight; k++)
    {
        for (int l = 0; l < nbBlockPerWidth; l++)
        {
            ...
            ssim_val += ((2 * avg_o * avg_r + C1) * (2 * sigma_ro + C2)) / ((avg_o * avg_o + avg_r * avg_r + C1) * (sigma_o * sigma_o + sigma_r * sigma_r + C2));
    ...
    ssim_val /= nbBlockPerHeight * nbBlockPerWidth;
...
void compute_quality_metrics(char * file1, char * file2, int block_size)
{
    ...
    {
        // cout << "Images must have the same dimensions: Img1: (" << height_o  << ", " << width_o << ") - Img2: (" << height_r << ", " << width_r << ")" << endl;
        return;
    }
...
void compute_quality_metrics_2(const Mat rgb_src_1, const Mat rgb_src_2, int block_size, double &psnr_val, double &ssim_val)
{
    ...
    Mat gray_src_1, gray_src_2;
    cvtColor(rgb_src_1, gray_src_1, CV_RGB2GRAY);
    cvtColor(rgb_src_2, gray_src_2, CV_RGB2GRAY);

    Mat dbl_src_1, dbl_src_2;
    gray_src_1.convertTo(dbl_src_1, CV_64F);
    gray_src_2.convertTo(dbl_src_2, CV_64F);

    int height_o = dbl_src_1.rows;
    int height_r = dbl_src_2.rows;
    int width_o = dbl_src_1.cols;
    int width_r = dbl_src_2.cols;
    ...
    ssim_val = ssim(dbl_src_1, dbl_src_2, block_size);
    psnr_val = psnr(dbl_src_1, dbl_src_2, block_size);
}
}
